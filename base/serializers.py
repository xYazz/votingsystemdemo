...
class VoteSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Vote
        fields = ('id', 'type', 'name', 'description', 'max_votes', 'start_date', 'end_date','candidates', 'private', 'code')

    def put(self, instance, validated_data):
        instance.type=validated_data.get('type', Vote.Type==instance.type)
        instance.name=validated_data.get('name', instance.name)
        instance.description=validated_data.get('description', instance.description)
        instance.max_votes=validated_data.get('max_votes', instance.max_votes)
...
class CreateVoteSerializer(serializers.ModelSerializer):
    class Meta:
        model = Vote
        fields = ('type', 'name', 'description', 'max_votes', 'start_date', 'end_date', 'private')
...
class CanVoteSerializer(serializers.ModelSerializer):
    ...
    def patch(self, instance, validated_data):
        instance.first_name=validated_data.get('can_vote', instance.can_vote)
        instance.save()
        return instance