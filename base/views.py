...
class EndedVoteList(APIView):
    ...
    def get(self, request, format=None):
        user = get_user(request)
        votes = self.queryset.filter(end_date__lt=now).filter(id__in=Candidate.objects.all().values_list('vote', flat=True))
        public = votes.filter(private=False)
        private = votes.filter(private=True, id__in= CanVote.objects.filter(voter=user, can_vote=True).values_list('vote', flat=True))
        available = public | private
    ...
    def patch(self, request, pk):
        ...
        try:
            ...
            if prev_can_vote.vote.owner==user:
                serializer = self.serializer_class(prev_can_vote, request.data, partial=True)
                if serializer.is_valid():
                    serializer.save()
...
class GetCandidate(APIView):
    ...
    def patch(self, request, pk):
        ...
        if prev_instance:
            serializer = self.serializer_class(prev_instance, request.data, partial=True)
            if serializer.is_valid():
                serializer.save()
                return Response(serializer.data, status=status.HTTP_202_ACCEPTED)
            return Response(serializer.errors, status=status.HTTP_208_ALREADY_REPORTED)
            ...
            try:
                ...
                for candidate in VoteSerializer(vote).data['candidates']:
                    ...
                    def get_value(name, idx):
                        ...
                        #     }
                        specific_candidate_data = len(vote_voters.filter(candidate=get_candidate, voter__in=CustomUser.objects.filter(**{name: idx})))
                        all_candidates_data = len(vote_voters.filter(candidate=get_candidate))
                        try:
